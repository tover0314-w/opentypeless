...
/** AES-GCM field protection for names, aliases, corrections, and app scopes. */
public final class LocalPersonalizationCipher {
    private static final String KEYSTORE = "AndroidKeyStore";
    private static final String KEY_ALIAS = "opentypeless_personalization_text_v1";
    private static final String PREFIX = "opentypeless-encrypted-personalization:v1:";
    private static final byte[] AAD =
            "OpenTypelessPersonalization:v1".getBytes(StandardCharsets.UTF_8);
    ...
    public LocalPersonalizationCipher() {
        this(null);
    }

    LocalPersonalizationCipher(SecretKey suppliedKey) {
    ...
    public String encrypt(String plaintext) {
        try {
            ...
            throw new IllegalStateException("Unable to protect personal vocabulary", error);
        }
    }
    ...
    public String decryptOrLegacy(String stored) {
        ...
        try {
            ...
            if (iv.length != EXPECTED_IV_BYTES || ciphertext.length < TAG_BITS / 8) {
                throw new IllegalArgumentException("Invalid protected value");
            }
            Cipher cipher = Cipher.getInstance("AES/GCM/NoPadding");
            ...
            throw new IllegalStateException("Unable to read personal vocabulary", error);
        }
    }
    ...
    private static byte[] decode(String value) {
        ...
        if (!encode(decoded).equals(value)) {
            throw new IllegalArgumentException("Non-canonical protected value");
        }
        return decoded;
    }
}
