...
Final 等待期间按键不被吞掉：已有 visible partial 按上项处理，按键同时明确取消迟到 Final；没有 partial 时
取消 voice composition 后处理按键。加密恢复只作为“⋮”中的可选兜底，不占据键盘主状态，也不阻止普通
按键；用户主动开始下一次录音时，旧恢复项由新录音替换。设置只选择协调意图，不能关闭敏感字段、Session、目标校验或
